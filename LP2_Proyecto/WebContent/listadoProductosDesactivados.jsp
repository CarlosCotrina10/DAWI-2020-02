					...
					<tbody>
						<c:forEach items="${milista}" var="x">
							<tr class="grilla_campo">
								<td>${x.idprod}</td>
								<td style="color: red;">${x.nomprod}</td>
								<td>${x.descripcion}</td>
								<td>${x.nomcat}</td>
								<td>${x.stock}</td>
								...
								<td><a href="crudProducto?btnes=e&cod=${x.idprod}&est=1"> <img
										alt="editar" src="img/activate1.png" height=21px width=21px
										title="Activar">
								</a></td>
							</tr>
						</c:forEach>
